...
const SUPPORTED_EXTENSIONS: &[&str] = &[
    ...
    "ac3", "dts", "thd", "truehd",
    // 容器格式
    "mka", "mkv", "mp4", "m4v", "mov", "webm", "asf",
    // 其他格式
    "amr", "au", "ra", "rm", "3gp",
    // 扩展支持
    "mid", "midi", "mod", "it", "s3m", "xm",
];
...
fn process_single_track(
    ...
    // 尝试读取元数据
    match Probe::open(path_buf).and_then(|p| p.read()) {
        Ok(tagged_file) => {
            let tag = tagged_file
                .primary_tag()
            ...
            // 跳过标题为空且时长小于30秒的文件
            if tag.and_then(|t| t.title()).is_none() && duration < 30_000.0 {
                return None;
            }
        ...
        Err(_) => {
            // 当元数据读取失败时，创建一个基本的 MusicTrack 对象
            // 这样至少可以在播放列表中看到这个文件
            Some(MusicTrack {
                id,
                path: path_str,
