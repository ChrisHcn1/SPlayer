...
export const useGeneralSettings = (): SettingConfig => {
  ...
  const checkFFmpegStatus = async () => {
    ...
    try {
      ...
      if (result.available) {
        window.$message.success("FFmpeg 已正确配置");
      } else {
  ...
  const cleanupTranscodeCache = async () => {
    if (!isElectron) return;
    window.$dialog.warning({
      title: "清理转码缓存",
      content: "确定要清理所有转码缓存吗？这将删除所有已转码的音频文件。",
      positiveText: "确定",
      negativeText: "取消",
      onPositiveClick: async () => {
        try {
          const result = await window.electron.ipcRenderer.invoke("audio-transcode-cleanup");
          if (result.success) {
            window.$message.success(`已清理 ${result.deletedCount} 个转码文件`);
            audioTranscodeManager.clearJobs();
          } else {
        ...
        items: [
          {
            ...
            label: "FFmpeg 路径",
            type: "text-input",
            description: "指定 FFmpeg 可执行文件的路径（留空则自动检测）",
            value: computed({
              get: () => settingStore.ffmpegPath,
              set: (v) => {
                settingStore.ffmpegPath = v;
                // 清除 FFmpeg 缓存，下次检查时会使用新路径
                audioTranscodeManager.resetFFmpegCache();
              },
            }),
          },
          {
            ...
            label: "FFmpeg 状态",
            type: "button",
            description: "检查系统是否已安装 FFmpeg",
            buttonLabel: "检查状态",
            action: checkFFmpegStatus,
          ...
          },
          {
            ...
            label: "启用音频转码",
            type: "switch",
            description: "自动将不支持的音频格式（如 APE、DTS、DSD）转码为 FLAC 格式播放",
            value: computed({
              get: () => settingStore.enableAudioTranscode,
              set: (v) => {
                settingStore.enableAudioTranscode = v;
                audioTranscodeManager.setPrefetchEnabled(v);
              },
            }),
          },
          {
            key: "cleanupTranscodeCache",
            label: "清理转码缓存",
            type: "button",
            description: "删除所有已转码的音频文件以释放磁盘空间",
            buttonLabel: "清理缓存",
            action: cleanupTranscodeCache,
