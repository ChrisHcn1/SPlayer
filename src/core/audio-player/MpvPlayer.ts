...
export class MpvPlayer extends EventTarget implements IPlaybackEngine {
  ...
  public init(): void {
    ...
    // window.electron.ipcRenderer.removeListener("mpv-property-change", this.handlePropertyChange);
    // window.electron.ipcRenderer.removeListener("mpv-file-loaded", this.handleFileLoaded);
    // window.electron.ipcRenderer.removeListener("mpv-playback-restart", this.handlePlaybackRestart);
    ...
    // 播放结束
    window.electron.ipcRenderer.on("mpv-ended", (...args: unknown[]) => {
