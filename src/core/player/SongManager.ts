...
import audioTranscodeManager from "@/core/resource/AudioTranscodeManager";
...
class SongManager {
  ...
  public prefetchNextSong = async (): Promise<AudioSource | undefined> => {
    try {
      ...
      if (nextSong.path) {
        ...
        // 预转码不支持的格式
        const needsTranscode = await this.checkNeedsTranscode(nextSong.path);
        if (needsTranscode && audioTranscodeManager.isFFmpegAvailable()) {
          // 获取当前歌曲的剩余播放时间
          const currentSong = playList[statusStore.playIndex];
          let remainingTimeMs = 10000; // 默认10秒
          ...
          // 提前10秒开始转码
          audioTranscodeManager.prefetchNextSong(nextSong, remainingTimeMs);
        }
        return;
      }
  ...
  public getAudioSource = async (song: SongType, forceSource?: string): Promise<AudioSource> => {
    console.log("🎵🎵🎵 [SongManager.getAudioSource] 函数被调用！歌曲信息:", song);
    const settingStore = useSettingStore();

    // 本地文件直接返回
    if (song.path && song.type !== "streaming") {
      console.log(`🎵 [${song.id}] 处理本地文件: ${song.path}`);
      ...
      // 检查 FFmpeg 是否可用
      const ffmpegAvailable = audioTranscodeManager.isFFmpegAvailable();
      console.log(`🔧 [${song.id}] FFmpeg 可用: ${ffmpegAvailable}`);

      // 检查是否需要转码
      const needsTranscode = await this.checkNeedsTranscode(song.path);
      console.log(`🔍 [${song.id}] 需要转码: ${needsTranscode}`);

      const transcodedPath = audioTranscodeManager.getTranscodedPath(song.id);
      if (transcodedPath) {
        console.log(`🔄 [${song.id}] 使用转码后的文件: ${transcodedPath}`);
        const encodedPath = transcodedPath.replace(/#/g, "%23").replace(/\?/g, "%3F");
        return { id: song.id, url: `file://${encodedPath}`, source: "local" };
      }

      // 如果文件需要转码但尚未转码，尝试转码
      if (needsTranscode && ffmpegAvailable) {
        console.log(`🔄 [${song.id}] 检测到不支持的格式，开始转码: ${song.path}`);
        ...
        console.warn(`⚠️ [${song.id}] 转码失败，使用原文件`);
      }

      const encodedPath = song.path.replace(/#/g, "%23").replace(/\?/g, "%3F");
      console.log(`📁 [${song.id}] 使用原文件: ${encodedPath}`);
      return { id: song.id, url: `file://${encodedPath}`, source: "local" };
    }
