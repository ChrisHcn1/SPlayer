...
onMounted(() => {
  ...
  ipc.on(TASKBAR_IPC_CHANNELS.SYNC_STATE, (...args: unknown[]) => {
    const payload = args[1] as SyncStatePayload;
    switch (payload.type) {
      case "full-hydration": {
  ...
  ipc.on(TASKBAR_IPC_CHANNELS.SYNC_TICK, (...args: unknown[]) => {
    const [currentTime, duration, offset] = args[1] as SyncTickPayload;
  ...
  ipc.on("taskbar:update-layout", (...args: unknown[]) => {
    const { isCenter } = args[1] as { isCenter: boolean };
