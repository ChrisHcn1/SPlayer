...
const getAllLocalMusic = debounce(
  async (showTip: boolean = false) => {
    ...
    // 监听批量track数据
    const tracksBatchHandler = (...args: unknown[]) => {
      const tracks = args[1] as Record<string, unknown>[];
      if (!loading.value || isCompleted) return;
      // 批量添加tracks
      receivedTracks.push(...tracks);
    };
    // 监听完成事件
    const completeHandler = (...args: unknown[]) => {
      const data = args[1] as SyncCompleteData;
...
onMounted(() => {
  ...
  const progressHandler = (...args: unknown[]) => {
    const payload = args[1] as { current: number; total: number };
