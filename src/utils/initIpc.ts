...
const initIpc = () => {
  try {
    ...
    // 播放模式切换
    window.electron.ipcRenderer.on("changeRepeat", (...args: unknown[]) => {
      const mode = args[1] as any;
      player.toggleRepeat(mode);
    });
    window.electron.ipcRenderer.on("toggleShuffle", (...args: unknown[]) => {
      const mode = args[1] as any;
      player.toggleShuffle(mode);
    });
    // 喜欢歌曲
    ...
    // 开启设置
    window.electron.ipcRenderer.on("openSetting", (...args: unknown[]) => {
      const type = args[1] as SettingType;
      const scrollTo = args[2] as string | undefined;
    ...
    window.electron.ipcRenderer.on("update-available", (...args: unknown[]) => {
      const info = args[1] as UpdateInfoType;
      closeUpdateStatus();
    ...
    // 更新下载进度
    window.electron.ipcRenderer.on("download-progress", (...args: unknown[]) => {
    ...
    // 协议数据
    window.electron.ipcRenderer.on("protocol-url", (...args: unknown[]) => {
