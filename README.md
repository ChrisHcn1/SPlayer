...
- 📁 本地音乐标签编辑及封面修改
  - ✅ 支持编辑歌曲信息（标题、艺术家、专辑、歌词等）
  - ✅ 自动修复元数据中的乱码问题
  - ✅ 支持多种音频格式（MP3、FLAC、WAV、DTS 等）
  - ✅ 智能歌词读取（优先从元数据读取，若为空则读取本地 .lrc 文件）
- ➕ 新建歌单及歌单编辑
- ❤️ 收藏 / 取消收藏歌单或歌手
- ☁️ 云盘音乐上传
- 📂 云盘内歌曲播放
...
- 📱 移动端基础适配
- 支持本地ffmpeg 进行音乐解码，建议自行安装 [ffmpeg](https://ffmpeg.org/download.html)，并配置系统变量 `PATH` 到 ffmpeg 安装目录
